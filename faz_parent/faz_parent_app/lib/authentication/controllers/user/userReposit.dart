...
class UserRepositoty extends GetxController{
  ...
  Future<void> saveUserModel(ProfModel user) async{
    try{
  ...
  Future<List<ProfModel>> fetchAllUsers() async{
    try{
      final documentSnapshot =  await _db.collection("prof").get();
      return documentSnapshot.docs.map((document) => ProfModel.fromSnapshot(document)).toList();
    }on FirebaseAuthException catch (e){
      throw AppFirebaseAuthException(e.code).message;
    }on FirebaseException catch (e){
      throw AppFirebaseException(e.code).message;
  ...
  Future<void> updateUserDetails(ProfModel updateUser) async{
    try{
