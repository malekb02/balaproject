...
class AddStudentScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: Text("إضافة تلميذ"),showBackArrow: false,),
      body: Padding(
            ...
            children: [
              SizedBox(
                ...
                child: Obx(() => DropdownButtonFormField<String>(
                  decoration: InputDecoration(labelText: 'اختر الولي',
                  ...
                  value: parentSelectController.selectedParent.value.id == ""
                      ? null
                      : parentSelectController.selectedParent.value.id,
                  items: parentSelectController.Parents.map((Parent) => DropdownMenuItem(
                    value: Parent.id,
                    child: Text(Parent.nom),
                  ))
                      .toList(),
                  onChanged: (value) {
                    if (value != null) parentSelectController.setTeacher(value);
                  },
              ...
              _buildTextField(controller.niveauSco, "المستوى الدراسي", true),
              _buildTextField(controller.age, "العمر", true, isNumber: true),
              _buildTextField(controller.noteImp, "ملاحظات", false),
              _buildTextField(controller.adresse, "العنوان", true),
              _buildTextField(controller.username, "اسم المستخدم", true),

              const SizedBox(height: 10),
              Obx(() => ListTile(
                title: Text("تاريخ الميلاد: ${DateFormat('yyyy-MM-dd').format(controller.dateNes.value)}"),
                trailing: Icon(Icons.calendar_today,color: Appcolors.primaryColor,),
                onTap: () => controller.pickDate((d) => controller.dateNes.value = d),
              )),

              Obx(() => ListTile(
                title: Text("تاريخ التسجيل: ${DateFormat('yyyy-MM-dd').format(controller.dateInsc.value)}"),
                trailing: Icon(Icons.calendar_today,color: Appcolors.primaryColor),
                onTap: () => controller.pickDate((d) => controller.dateInsc.value = d),
              )),
            ...
            Expanded(
                child: ElevatedButton(
                    onPressed: () {
                      ...
                      controller.submitStudent(_formKey,parentSelectController);
                    },
...
class StudentAddController extends GetxController {
  ...
  final numParent = TextEditingController();
  ...
  Future<void> submitStudent(GlobalKey<FormState> formKey,parentSelect) async {
    ...
    StudentController.instance.AddNewStudent(student,parentSelect.selectedParent);
